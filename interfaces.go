...
type LotteryDrawer interface {
	...
	DrawMultipleInRange(ctx context.Context, lockKey string, min, max, count int, progressCallback ProgressCallback) (*MultiDrawResult, error)

	// DrawMultipleFromPrizes draws multiple prizes from the given prize pool
	DrawMultipleFromPrizes(ctx context.Context, lockKey string, prizes []Prize, count int, progressCallback ProgressCallback) (*MultiDrawResult, error)
